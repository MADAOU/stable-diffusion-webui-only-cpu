...
def train_embedding(embedding_name, learn_rate, batch_size, data_root, log_directory, training_width, training_height, steps, create_image_every, save_embedding_every, template_file, save_image_with_stored_embedding, preview_from_txt2img, preview_prompt, preview_negative_prompt, preview_steps, preview_sampler_index, preview_cfg_scale, preview_seed, preview_width, preview_height):
    ...
    validate_train_inputs(embedding_name, learn_rate, batch_size, data_root, template_file,
                          steps, save_embedding_every, create_image_every, log_directory, name="embedding")
    ...
    for i, entries in pbar:
        ...
        pbar.set_description(
            f"[Epoch {epoch_num}: {epoch_step+1}/{len(ds)}]loss: {losses.mean():.7f}")
        ...
        if embedding_dir is not None and steps_done % save_embedding_every == 0:
            ...
            save_embedding(embedding, checkpoint, embedding_name_every,
        ...
        if images_dir is not None and steps_done % create_image_every == 0:
            ...
            if save_image_with_stored_embedding and os.path.exists(last_saved_file) and embedding_yet_to_be_embedded:
                ...
                data = torch.load(
                    last_saved_file, map_location=torch.device('cpu'))
...
"""
    ...
    filename = os.path.join(
        shared.cmd_opts.embeddings_dir, f'{embedding_name}.pt')
    save_embedding(embedding, checkpoint, embedding_name,
